...
test.describe('Mobile Experience', () => {
  ...
  test.skip('should open and close mobile navigation', async ({ page }) => {
    ...
    await expect(
      mobileNavigation.getByRole('link', { name: 'Jobs' }),
    ).toBeVisible();
    await expect(
      mobileNavigation.getByRole('link', { name: 'Companies' }),
    ).toBeVisible();
  ...
  test('should show mobile filters', async ({ page }) => {
    ...
    if (await filterButton.isVisible()) {
      ...
      await expect(
        page.getByRole('heading', { name: /job type/i }),
      ).toBeVisible();
      ...
      const hasActionButton =
        (await applyButton.isVisible()) || (await closeButton.isVisible());
  ...
  test('should handle mobile form inputs', async ({ page }) => {
    ...
    const applySection = page.getByRole('heading', {
      name: /apply for this position/i,
    });
