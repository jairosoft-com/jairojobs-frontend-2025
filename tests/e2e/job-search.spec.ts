...
test.describe('Job Search and Filtering', () => {
  ...
  test('should display all jobs by default', async ({ page }) => {
    ...
    await expect(jobCards).toHaveCount(20); // Default page size is 20
  });
  ...
  test.skip('should filter by job type', async ({ page }) => {
    ...
    const jobTypes = page.locator(
      '[data-testid="job-card"] div:has-text("Full Time")[class*="rounded-md"][class*="border"]',
    );
  ...
  test.skip('should filter by experience level', async ({ page }) => {
    ...
    // Badge component renders a div with inline classes, not .badge class
    const seniorJobs = page.locator(
      '[data-testid="job-card"] div:has-text("senior")[class*="rounded-md"][class*="border"]',
    );
  ...
  test.skip('should filter by multiple criteria', async ({ page }) => {
    ...
    const locationCheckbox =
      jobsPage.filters.location.getByLabel('San Francisco, CA');
    ...
    expect(jobCards).toBeLessThanOrEqual(20); // Default page size is 20
  });
  ...
  test('should search by location', async ({ page }) => {
    ...
    const locations = page.locator(
      '[data-testid="job-card"] [data-testid="job-location"]',
    );
    const locationTexts = await locations.allTextContents();

    // At least one job should be in San Francisco or Remote
    const relevantJobs = locationTexts.filter(
      loc =>
  ...
  test.skip('should handle pagination', async ({ page }) => {
    ...
    const firstJobTitle = await jobsPage.jobCards
      .first()
      .locator('h3')
      .textContent();
    ...
    const newFirstJobTitle = await jobsPage.jobCards
      .first()
      .locator('h3')
      .textContent();
  ...
  test.skip('should persist filters in URL', async ({ page }) => {
    ...
    const seniorCheckbox =
      jobsPage.filters.experienceLevel.getByLabel('Senior Level');
